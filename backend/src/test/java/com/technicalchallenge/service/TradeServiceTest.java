...
class TradeServiceTest {
     ...
     */
    @Test
    // correctly filters trades by counterparty name.
    void testSearchTrades_FilterByCounterparty() {
        // SETUP: Create search criteria with a counterparty filter
        SearchCriteriaDTO criteria = new SearchCriteriaDTO();
        criteria.setCounterparty("BigBank");

        // Create a Trade entity with the matching counterparty
        ...
        // Mock the repository telling Mockito to return only one trade by passing a
        // list with a single element when any Specification is used
        // Being explicit about which overload I want to mock the Specification version
        // to fix the ambiguous error

        when(tradeRepository.findAll(any(Specification.class))).thenReturn(Arrays.asList(trade));

        when(tradeMapper.toDto(any(Trade.class))).thenReturn(dto);// returns the specific TradeDTO instance referenced
                                                                  // by dto

        // Act: Call the service method set fields on dto (like
        // dto.setCounterpartyName("BigBank")), those values will be present in the
        // returned object
        List<TradeDTO> result = tradeService.searchTrades(criteria);

        // Assert: Verify the result contains the expected trade
        assertEquals(1, result.size());
        assertEquals("BigBank", result.get(0).getCounterpartyName());
    }

    /**
     * Tests the filterTrades method with pagination to verify that filtering by
     * counterparty name works.
     * Sets up a Trade entity with a specific counterparty, mocks repository and
     * mapper to return a paginated result,
     * and asserts that the returned TradeDTO list contains the expected
     * counterparty name.
     */
    @Test
    void testFilterTrades_FilterByCounterparty() {
        ...
        when(tradeRepository.findAll(any(Specification.class), any(org.springframework.data.domain.Pageable.class)))
                .thenReturn(tradePage);

        // Act: Call the service method
        /*
         * getContent() method extracts the actual list of TradeDTO objects from the
         * page, ignoring pagination metadata like total pages,total element
         */
        List<TradeDTO> result = tradeService.filterTrades(criteria, 0, 10).getContent();

        // Assert: Verify the result contains the expected trade
        assertEquals(1, result.size());
        // Using .getContent() extracts just the list of items for that page, not all
        // filtered results. So, the test is verifying that filtering works and that
        // pagination returns the correct subset of results.
        assertEquals("BigBank", result.get(0).getCounterpartyName());
    }

    /**
     * Tests the rsqlSearch method with a complex RSQL query (multiple conditions).
     * Sets up a Trade entity matching both counterparty and trade status, mocks
     * repository and mapper,
     * and asserts that the returned TradeDTO matches all query conditions.
     */
    @Test
    void testRsqlSearch_ComplexQuery() {
        // Example complex RSQL query: filter by counterparty and trade status
        String rsqlQuery = "counterparty.name==BigBank;tradeStatus.tradeStatus==LIVE";

        // Create a Trade entity that matches both conditions
        Trade trade = new Trade();
        trade.setTradeId(4L);
        Counterparty cp = new Counterparty();
        cp.setName("BigBank");
        ...
        // Act: Call the RSQL search method
        List<TradeDTO> result = tradeService.searchTradesRsql(rsqlQuery);
