...
public class UserController {
    ...
    @GetMapping
    @Operation(summary = "Get all users", description = "Retrieves a list of all users in the system with their profile information and privileges")
    @ApiResponses(value = {
    ...
    @Operation(summary = "Get user by login ID", description = "Retrieves a specific user by their unique login identifier")
    @ApiResponses(value = {
            @ApiResponse(responseCode = "200", description = "User found and returned successfully", content = @Content(mediaType = "application/json", schema = @Schema(implementation = UserDTO.class))),
    ...
    public ResponseEntity<UserDTO> getUserByLoginId(
            @Parameter(description = "Unique login identifier of the user", required = true) @PathVariable("loginId") String loginId) {
    ...
    @PostMapping
    @Operation(summary = "Create new user", description = "Registers a new user in the system with the provided profile information")
    @ApiResponses(value = {
    ...
    @PutMapping("/{id}")
    @Operation(summary = "Update user", description = "Updates the profile information of an existing user")
    @ApiResponses(value = {
    ...
    @GetMapping("/values")
    @Operation(summary = "Get all trade type values", description = "Retrieves a list of all trade type values associated with users")
    @ApiResponses(value = {
            @ApiResponse(responseCode = "200", description = "Successfully retrieved all trade type values", content = @Content(mediaType = "application/json", schema = @Schema(implementation = String.class))),
            @ApiResponse(responseCode = "500", description = "Internal server error")
    })
