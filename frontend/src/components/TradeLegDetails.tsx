 ...
 */
const TradeLegDetails: React.FC<TradeLegDetailsProps> = ({
  leg,
  mode,
  onFieldChange,
}) => {
  ...
  return (
    <div className="mt-4 grid p-0 bg-indigo-100 rounded shadow text-sm w-fit ml-1">
      {TRADE_LEG_FIELDS.map((field) => {
        let isDisabled =
          mode !== "edit" || DISABLED_LEG_FIELDS.includes(field.key);
        ...
        return (
          <div
            className="flex flex-row gap-x-10 items-center mb-1"
            key={field.key}
          >
            ...
            <div className="flex flex-row justify-end rounded px-2 py-1 h-fit w-full">
              <FieldRenderer
                field={field}
                value={leg[field.key] as string | number | null | undefined} // refactored to match backend TradeLeg data types
                disabled={isDisabled}
