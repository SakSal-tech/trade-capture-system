...
class StaticStore {
  ...
  // userValues stores dropdown option objects { value: string, label: string }
  // where `value` is the user's id (string) and `label` is the display name.
  // This lets the UI send numeric user ids as dropdown values while
  // keeping a readable label for users to select.
  _userValues: Array<{ value: string; label: string }> = [];
  ...
  set userValues(v: Array<{ value: string; label: string }>) {
    this._userValues = v;
  }
  ...
  async fetchAllStaticValues() {
    ...
    try {
      ...
      // Normalize users list into { value, label } option objects.
      // Backend may return an array of strings or objects; handle both.
      ...
      const normalizedUsers: Array<{ value: string; label: string }> =
        Array.isArray(rawUsers)
          ...
          : [];
      this.userValues = normalizedUsers;
