...
const USER_FIELDS = [
  ...
  { key: "loginId", label: "User Id", type: "input" },
  {
    ...
    label: "Password",
    type: "input",
    hideIf: (user: ApplicationUser) => !!user.id,
  },
  { key: "active", label: "Active", type: "checkbox" },
  {
    ...
    type: "dropdown",
    options: () =>
      (staticStore.userTypeCache ?? []).map((u) => ({
        value: u.userType,
        label: u.userType,
      })),
  },
...
export const SingleUserModal: React.FC<UserDetailModalProps> = observer(
  (props) => {
    ...
    return (
      <div className="bg-violet-50 mt-10 w-full max-w-xl mx-auto rounded-lg shadow-lg p-8 flex flex-col gap-4 items-center">
        ...
        <div className="flex flex-col gap-4 w-full max-w-md items-center">
          {USER_FIELDS.filter((f) => !(f.hideIf && f.hideIf(user))).map(
            (field) => {
              if (field.type === "input") {
                return (
                  <div
                  ...
                  >
                    <Label className="w-1/3 text-right pr-2">
                      {field.label}
                    </Label>
                    <Input
                      type={field.key === "password" ? "password" : "text"}
                      value={String(
                        user[field.key as keyof ApplicationUser] ?? ""
                      )}
                      onChange={(e: React.ChangeEvent<HTMLInputElement>) =>
                        handleFieldChange(
                          field.key as keyof ApplicationUser,
                          e.target.value
                        )
                      }
        ...
        <div className="flex flex-row gap-4 mt-6 justify-center w-full">
          <Button variant="primary" type="button" onClick={props.onClickSave}>
            Save
          </Button>
