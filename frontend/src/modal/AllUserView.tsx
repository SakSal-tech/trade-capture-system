...
interface AllUserViewProps {
  setView?: (newView: string, user?: ApplicationUser) => void;
}
...
export const AllUserView: React.FC<AllUserViewProps> = observer(
  ({ setView }) => {
    ...
    const [currentUser, setCurrentUser] = useState<ApplicationUser | null>(
      null
    );
    ...
    const handleEditUser = () => {
      if (currentUser) {
        ...
        if (setView) {
          ...
          setSnackbarOpen(true);
          setSnackbarMsg(
            `Opening edit form for user: ${currentUser.firstName} ${currentUser.lastName}`
          );
    ...
    return (
      <div
        className={
          "flex flex-col justify-start min-h-full w-full justify-items-center mt-2"
        }
      >
        <div className={"flex flex-col justify-center justify-items-center"}>
          <div
            className={
              "flex text-2xl justify-self-center mb-2 flex-row justify-center shadow rounded bg-indigo-100"
            }
          >
            User Details
          </div>
